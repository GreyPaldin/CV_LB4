...
## Задача 
&ensp; В рамках данной работы было необходимо обучить модель с любой архитектурой на наш выбор. Задача модели - находить, определять несколько классов объектов на изображении.
...
&ensp; Рссмотрим поэтапно работу модели. Первоначльно изобржение разбивается на фрагменты с сеткой 7x7 (стандартное значение), после чего если объект попдает в какую-то ячейку, то ячейка начинает отвечать за объект. Каждая из ячеек способна определить, что в ней находится, после чего дання информция объеденяется, чтобы можно было сделать вывод о том, что изображено.

&ensp; Внутри каждой из 49 ячеек находится два boxs'а, внутри кааждого из которых есть вектор, состоящий из 20 знчений - так как первые модели строились для определения 20 классов, каждое значение - принадлежность к классу.

&ensp; Модель основан на модели GoogleNet и состоит из 24 свёрточных слоёв и двух полносвязаных слоёв (всего 26) для прогнзирвония боксов. Финальный слой использует линейную функцию активаци.

&ensp; В качестве функции потерь применяется сумма квадртов ошибки для каждого объекта в каждой ячейки. Она лего оптимизируема, но при этом

## Архитектура модели  

### Backbone: CSPDarknet  
&ensp;Основная сеть для извлечения признаков, построенная на принципах **CSP (Cross Stage Partial)**.  
- **CSP-блоки**:  
...
### Neck: PAN-FPN  
&ensp;Слой для комбинирования признаков из разных уровней сети:  
...
### Head: Anchor-Free  
&ensp;Головная часть модели, ответственная за предсказание координат и классов объектов:  
- **Прямое предсказание**:  
...
### Функции потерь  
- **Box Loss**: Использует **CIoU (Complete Intersection over Union)**, учитывающий:  
  - Перекрытие между предсказанным и истинным bounding box.  
  - Расстояние между центрами.  
  - Соотношение сторон.  
- **Class Loss**: Бинарная кросс-энтропия для классификации объектов.  
- **Objectness Loss**: Оценивает уверенность модели в наличии объекта.  

### Аугментации данных  
- **Mosaic**: Сборка изображения из 4 случайных фрагментов для обучения на сложных сценах.  
- **MixUp**: Наложение двух изображений для увеличения разнообразия данных.  
- **HSV-коррекция**: Изменение оттенка, насыщенности и яркости для улучшения устойчивости к освещению.  

### Оптимизация  
- **OneCycleLR**: Стратегия динамического изменения learning rate для ускорения сходимости.  
- **Эволюция гиперпараметров**: Автоматический подбор параметров (например, веса регуляризации). 
...
&ensp; Процесс обучения на более чем 22500 изображений с двумя классами заняло около 1 часа на графических ускорителей сервиса Colab (на графическом процессоре Т4).
